...
export class SimulatorComponent implements OnInit {
  ...
  pendingScans: { barcode: string; productName: string }[] = [];
  currentScanIndex: number = 0;
  ...
  ========================================= */
  generateCustomerRequest(mood: string): { request: string, message: string, patience: number, barcodes: string[], pendingScans: { barcode: string, productName: string }[] } {
    let patience = 5;
    ...
    const pendingScans: { barcode: string, productName: string }[] = [];
    orderItems.forEach(item => {
      itemsList += `\n• ${item.qty} unidad(es) de ${item.product.name}`;
      if (item.product.barcode) {
        for (let q = 0; q < item.qty; q++) {
          barcodes.push(item.product.barcode);
          pendingScans.push({ barcode: item.product.barcode, productName: item.product.name });
        }
      }
    });
  ...
  generateCustomerScenario(): void {
    ...
    this.pendingScans = data.pendingScans;
    this.currentScanIndex = 0;
