...
export class StatisticsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      if (params['tab']) {
        ...
        if (this.currentTab === 'rfid') this.loadRfidRequests();
      }
    });
  }
    ...
    this.userService.reviewRfidRequest(req.id, 'rejected', this.currentAdminId, 'Rechazado por administrador').subscribe({
      next: () => {
        ...
        notifs.push({
          id: String(Date.now()),
          role: targetRole,
          message: `❌ Tu solicitud de tarjeta RFID (${req.rfidUid}) fue rechazada.`,
          actionText: 'Ver Perfil',
          route: '/profile',
          read: false
        });
  ...
  mapRole(dbRole: string): string {
    ...
    if (r === 'APPRENTICE' || r === 'APRENDIZ') return 'APRENDIZ';
